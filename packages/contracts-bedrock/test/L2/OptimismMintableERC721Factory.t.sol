...
contract OptimismMintableERC721Factory_Test is Bridge_Initializer {
    ...
    function test_constructor_succeeds() external view {
        assertEq(erc721Factory.BRIDGE(), address(l2ERC721Bridge));
        assertEq(erc721Factory.REMOTE_CHAIN_ID(), deploy.cfg().l1ChainID());
    }
    ...
    function test_createOptimismMintableERC721_succeeds() external {
        ...
        vm.expectEmit(address(erc721Factory));
        emit OptimismMintableERC721Created(local, remote, alice);

        // Create the token.
        vm.prank(alice);
        OptimismMintableERC721 created =
            OptimismMintableERC721(erc721Factory.createOptimismMintableERC721(remote, "L2Token", "L2T"));

        // Token address should be correct.
        assertEq(address(created), local);

        // Should be marked as created by the factory.
        assertEq(erc721Factory.isOptimismMintableERC721(address(created)), true);
    ...
    function test_createOptimismMintableERC721_sameTwice_reverts() external {
        ...
        vm.prank(alice);
        erc721Factory.createOptimismMintableERC721(remote, "L2Token", "L2T");
    ...
    function test_createOptimismMintableERC721_zeroRemoteToken_reverts() external {
        ...
        erc721Factory.createOptimismMintableERC721(address(0), "L2Token", "L2T");
    }
        ...
        returns (address)
    {
        ...
        bytes32 hash = keccak256(abi.encodePacked(bytes1(0xff), address(erc721Factory), salt, keccak256(bytecode)));
