...
contract L1Block is ISemver, IGasToken {
    ...
    /// @notice
    bytes32 internal constant SEQUENCER_FEE_VAULT_CONFIG_SLOT = bytes32(uint256(keccak256("opstack.sequencerfeevaultconfig")) - 1);
    /// @notice
    bytes32 internal constant L1_CROSS_DOMAIN_MESSENGER_ADDRESS_SLOT = bytes32(uint256(keccak256("opstack.l1crossdomainmessengeraddress")) - 1);
    /// @notice
    bytes32 internal constant L1_ERC_721_BRIDGE_ADDRESS_SLOT = bytes32(uint256(keccak256("opstack.l1erc721bridgeaddress")) - 1);
    /// @notice
    bytes32 internal constant L1_STANDARD_BRIDGE_ADDRESS_SLOT = bytes32(uint256(keccak256("opstack.l1standardbridgeaddress")) - 1);
    /// @notice
    ...
    function setHolocene() external {
        ...
        Storage.setAddress(L1_CROSS_DOMAIN_MESSENGER_ADDRESS_SLOT, address(ICrossDomainMessenger(Predeploys.L2_CROSS_DOMAIN_MESSENGER).otherMessenger()));
        Storage.setAddress(L1_STANDARD_BRIDGE_ADDRESS_SLOT, address(IStandardBridge(payable(Predeploys.L2_STANDARD_BRIDGE)).otherBridge()));
        Storage.setAddress(L1_ERC_721_BRIDGE_ADDRESS_SLOT, address(IERC721Bridge(Predeploys.L2_ERC721_BRIDGE).otherBridge()));
        Storage.setUint(REMOTE_CHAIN_ID_SLOT, IOptimismMintableERC721Factory(Predeploys.L2_ERC721_BRIDGE).remoteChainId());
    }

    /// @notice
    function baseFeeVaultConfig() public view returns (address recipient_, uint256 amount_, IFeeVault.WithdrawalNetwork network_) {
        (recipient_, amount_, network_) = Encoding.decodeFeeVaultConfig(Storage.getBytes32(BASE_FEE_VAULT_CONFIG_SLOT));
    }

    /// @notice
    function l1FeeVaultConfig() public view returns (address recipient_, uint256 amount_, IFeeVault.WithdrawalNetwork network_) {
        (recipient_, amount_, network_) = Encoding.decodeFeeVaultConfig(Storage.getBytes32(L1_FEE_VAULT_CONFIG_SLOT));
    }

    /// @notice
    function sequencerFeeVaultConfig() public view returns (address recipient_, uint256 amount_, IFeeVault.WithdrawalNetwork network_) {
